describe("objeto FireBall",function(){
	...
	it ("objeto FireBallI.step", function(){
		...
		runs(function(){
			...
			expect (miboard.remove.calls[0].args[0]).toBe (miFireBallI); // me aseguro del correcto paso de parmetros.
	...
	it ("objeto FireBallD.step", function(){
		...
		runs(function(){
			expect (miboard.remove).toHaveBeenCalled(); // se debera haber llamado a remove
			expect (miboard.remove.calls[0].args[0]).toBe (miFireBallD); // me aseguro del correcto paso de parmetros.
			expect (miboard.remove.length).toBe(0); // aunque se halla intentado aadir a removed no se puede puesto que no existe.
		});
